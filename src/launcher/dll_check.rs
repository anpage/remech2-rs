...
impl DllCheck {
    ...
    fn start_download(&mut self) {
        ...
        let status = Arc::clone(&self.downloading_status);

        std::thread::spawn(move || {
            let download_file = |url: &str| -> Result<Vec<u8>, String> {
                ...
                loop {
                    let n =
                        std::io::Read::read(&mut reader, &mut chunk).map_err(|e| e.to_string())?;
                    if n == 0 {
                        break;
                    }
                    ...
                    downloaded += n as u64;
                    *status.lock().unwrap() =
                        DownloadStatus::Downloading(downloaded as f32 / total_size as f32);
                }
