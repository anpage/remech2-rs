...
pub unsafe fn hook_functions(base_address: usize) -> Result<()> {
    ...
    AUDIO_SUBSYSTEM_DESTRUCTOR_HOOK = {
        let target: AudioSubsystemDestructorFunc = std::mem::transmute(base_address + 0x0003cf5d);
    ...
    AUDIO_SAMPLE_GET_IS_PLAYING_HOOK = {
        let target: AudioSampleGetIsPlayingFunc = std::mem::transmute(base_address + 0x0003d77e);
    ...
    AUDIO_SAMPLE_SET_LOOP_COUNT_HOOK = {
        let target: AudioSampleSetLoopCountFunc = std::mem::transmute(base_address + 0x0003d842);
    ...
    MIDI_SEQUENCE_CONSTRUCTOR_HOOK = {
        let target: MidiSequenceConstructorFunc = std::mem::transmute(base_address + 0x0003d198);
    ...
    MIDI_SEQUENCE_SET_LOOP_COUNT_HOOK = {
        let target: MidiSequenceSetLoopCountFunc = std::mem::transmute(base_address + 0x0003d25c);
